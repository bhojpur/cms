(function(factory) {
    ...
    let URL = window.URL || window.webkitURL,
        ...
        EVENT_HIDDEN = 'hidden.bhojpur.modal',
        // Classes
        CLASS_TOGGLE = '.bhojpur-cropper__toggle',
        CLASS_CANVAS = '.bhojpur-cropper__canvas',
        CLASS_WRAPPER = '.bhojpur-cropper__wrapper',
        CLASS_OPTIONS = '.bhojpur-cropper__options',
        CLASS_SAVE = '.bhojpur-cropper__save',
        CLASS_DELETE = '.bhojpur-cropper__toggle--delete',
        CLASS_CROP = '.bhojpur-cropper__toggle--crop',
        CLASS_UNDO = '.bhojpur-fieldset__undo',
        HIDDEN_DATA_INPUT = 'input[name="BhojpurResource.MediaOption"]:hidden';
    ...
    function BhojpurCropper(element, options) {
        this.$element = $(element);
        this.options = $.extend(true, {}, BhojpurCropper.DEFAULTS, $.isPlainObject(options) && options);
        this.data = null;
        this.init();
    }

    BhojpurCropper.prototype = {
        constructor: BhojpurCropper,
        ...
        build: function() {
            ...
            this.wrap();
            this.$modal = $(replaceText(BhojpurCropper.MODAL, replaceTexts)).appendTo('body');
        },
        ...
        wrap: function() {
            ...
            if ($img.length) {
                $list.find('li').append(BhojpurCropper.TOGGLE);
                $img.wrap(BhojpurCropper.CANVAS);
                this.center($img);
            } else {
        ...
        click: function(e) {
            ...
            if ($target.closest(CLASS_DELETE).length) {
                ...
                this.$parent.find('.bhojpur-fieldset').append($alert);
            }
        ...
        read: function(e) {
            let files = e.target.files,
                file,
                $list = this.$list,
                $alert = this.$parent.find('.bhojpur-fieldset__alert');
            ...
            if (files && files.length) {
                ...
                if (/^image\//.test(file.type) && URL) {
                    ...
                    this.$parent.find('.bhojpur-medialibrary__image-desc').show();
                } else {
                    $list.empty().html(BhojpurCropper.FILE_LIST.replace('{{filename}}', file.name));
                }
            }
        },
        ...
        start: function() {
            ...
            $clone.cropper({
                ...
                ready: function() {
                    $modal
                        .find('.bhojpur-cropper__options-toggle')
                        .on(EVENT_CLICK, function() {
                            $modal.find('.bhojpur-cropper__options-input').prop('checked', $(this).prop('checked'));
                        })
                        .prop('checked', true);
        ...
        stop: function() {
            this.$modal
                .trigger('disable.bhojpur.material')
                ...
                .each(function() {
                    ...
                    if (resolution) {
                        ...
                        if (width / height === aspectRatio) {
                            list.push(
                                '<label>' +
                                    '<input class="bhojpur-cropper__options-input" type="checkbox" name="' +
                                    name +
                                    '" checked> ' +
                                    '<span>' +
                                    name +
    ...
    BhojpurCropper.TOGGLE = `<div class="bhojpur-cropper__toggle">
    ...
    BhojpurCropper.ALERT = `<div class="bhojpur-fieldset__alert">
            <button class="mdl-button mdl-button--accent bhojpur-fieldset__undo" type="button">Undo delete</button>
        </div>`;

    BhojpurCropper.CANVAS = '<div class="bhojpur-cropper__canvas"></div>';
    BhojpurCropper.LIST = '<ul><li><img></li></ul>';
    BhojpurCropper.FILE_LIST = `<div class="bhojpur-file__list-item">
                                <span><span>{{filename}}</span></span>
                                <div class="bhojpur-cropper__toggle">
                                    <div class="bhojpur-cropper__toggle--delete"><i class="material-icons">delete</i></div>
                                </div>
                            </div>`;
    BhojpurCropper.MODAL = `<div class="bhojpur-modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    BhojpurCropper.plugin = function(option) {
        return this.each(function() {
            ...
            if (!data) {
                ...
                $this.data(NAMESPACE, (data = new BhojpurCropper(this, options)));
            }
    ...
    $(function() {
        let selector = '.bhojpur-file__input',
            options = {
                parent: '.bhojpur-file',
                output: '.bhojpur-file__options',
                list: '.bhojpur-file__list',
                key: 'CropOptions'
            };

        $(document)
            .on(EVENT_ENABLE, function(e) {
                BhojpurCropper.plugin.call($(selector, e.target), options);
            })
            .on(EVENT_DISABLE, function(e) {
                BhojpurCropper.plugin.call($(selector, e.target), 'destroy');
            })
            .triggerHandler(EVENT_ENABLE);
    });

    return BhojpurCropper;
});