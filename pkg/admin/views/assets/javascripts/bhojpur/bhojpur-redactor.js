(function(factory) {
    ...
    let NAMESPACE = "bhojpur.redactor",
      ...
      CLASS_CROPPER_TOGGLE = ".bhojpur-cropper__toggle--redactor";
    ...
    function BhojpurRedactor(element, options) {
      this.$element = $(element);
      this.options = $.extend(
        true,
        {},
        BhojpurRedactor.DEFAULTS,
        $.isPlainObject(options) && options
      );
      this.init();
    }
  
    BhojpurRedactor.prototype = {
      constructor: BhojpurRedactor,
      ...
      init: function() {
        ...
        this.$parent = $parent;
        this.$button = $(BhojpurRedactor.BUTTON);
        this.$modal = $(replaceText(BhojpurRedactor.MODAL, options.text)).appendTo(
          "body"
        );
        this.bind();
      },
      ...
      destroy: function() {
        this.unbind();
        this.$modal.bhojpurModal("hide").remove();
    ...
    BhojpurRedactor.BUTTON = `<div class="bhojpur-redactor__image--buttons">
    ...
    BhojpurRedactor.MODAL = `<div class="bhojpur-modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    BhojpurRedactor.plugin = function(option) {
      return this.each(function() {
        ...
        if (!data) {
          ...
          config = {
            ...
            toolbarFixedTarget:
              !$this.closest(".bhojpur-slideout").length &&
              !$this.closest(".bhojpur-bottomsheets").length
            ...
            callbacks: {
              started: function() {
                ...
                if (isInSlideout) {
                  if ($this.closest(".bhojpur-bottomsheets").length != 0) {
                    toolbarFixedTarget = $this.closest(".bhojpur-page__body");
                    toolbarFixedTopOffset = $this
                      .closest(".bhojpur-page__body")
                      .offset().top;
                  } else {
                    toolbarFixedTarget = ".bhojpur-slideout__body";
                    toolbarFixedTopOffset = $(".bhojpur-slideout__header").height();
                  }
                } else {
                  toolbarFixedTarget = ".bhojpur-layout main.bhojpur-page";
                  toolbarFixedTopOffset =
                    toolbarFixedTopOffset +
                    $(toolbarFixedTarget)
                      .find(".bhojpur-page__header")
                      .height();
                }
                ...
                $this.data(
                  NAMESPACE,
                  (data = new BhojpurRedactor($this, {
                    remote: $this.data("cropUrl"),
                    text: $this.data("text"),
                    parent: ".bhojpur-field",
                    toggle: ".bhojpur-cropper__toggle--redactor",
    ...
    $(function() {
      var selector = 'textarea[data-toggle="bhojpur.redactor"]';
  
      $(document)
        .on(EVENT_DISABLE, function(e) {
          BhojpurRedactor.plugin.call($(selector, e.target), "destroy");
        })
        .on(EVENT_ENABLE, function(e) {
          BhojpurRedactor.plugin.call($(selector, e.target));
        })
        .triggerHandler(EVENT_ENABLE);
    });
  
    return BhojpurRedactor;
  });