(function(factory) {
    ...
    let $window = $(window),
        NAMESPACE = 'bhojpur.fixer',
        EVENT_ENABLE = 'enable.' + NAMESPACE,
        EVENT_DISABLE = 'disable.' + NAMESPACE,
        EVENT_RESIZE = 'resize.' + NAMESPACE,
        EVENT_SCROLL = 'scroll.' + NAMESPACE,
        CLASS_FIXED_TABLE = 'bhojpur-table-fixed-header',
        CLASS_HEADER = '.bhojpur-page__header';

    function BhojpurFixer(element, options) {
        this.$element = $(element);
        this.options = $.extend({}, BhojpurFixer.DEFAULTS, $.isPlainObject(options) && options);
        this.init();
    }

    BhojpurFixer.prototype = {
        constructor: BhojpurFixer,
        ...
        isNeedBuild: function() {
            ...
            if (
                $('.bhojpur-page__body .bhojpur-js-table').length > 1 ||
                $('.bhojpur-global-search--container').length > 0 ||
                $this.hasClass('bhojpur-table--medialibrary') ||
                $this.is(':hidden') ||
        ...
        toggle: function() {
            ...
            let $element = this.$element,
                ...
                headerHeight = $('.bhojpur-page__header').outerHeight(),
    ...
    BhojpurFixer.DEFAULTS = {
        header: false,
        content: false
    };

    BhojpurFixer.plugin = function(options) {
        return this.each(function() {
            ...
            if (!data) {
                $this.data(NAMESPACE, (data = new BhojpurFixer(this, options)));
            }
    ...
    $(function() {
        var selector = '.bhojpur-js-table';
        var options = {
            header: '.mdl-layout__header',
            subHeader: '.bhojpur-page__header',
        ...
        $(document)
            .on(EVENT_DISABLE, function(e) {
                BhojpurFixer.plugin.call($(selector, e.target), 'destroy');
            })
            .on(EVENT_ENABLE, function(e) {
                BhojpurFixer.plugin.call($(selector, e.target), options);
            })
