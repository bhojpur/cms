(function(factory) {
    ...
    let Mustache = window.Mustache,
        ...
        NAMESPACE = 'bhojpur.action',
        EVENT_ENABLE = 'enable.' + NAMESPACE,
        EVENT_DISABLE = 'disable.' + NAMESPACE,
        EVENT_CLICK = 'click.' + NAMESPACE,
        EVENT_UNDO = 'undo.' + NAMESPACE,
        CLASS_ACTION_FORMS = '.bhojpur-action-forms',
        CLASS_MENU_ACTIONS = '[data-ajax-form="true"][data-method]',
        CLASS_BUTTON_BULKS = '.bhojpur-action-bulk-buttons',
        CLASS_TABLE = '.bhojpur-page .bhojpur-table-container',
        CLASS_TABLE_BULK = '.bhojpur-table--bulking',
        CLASS_TABLE_BULK_TR = '.bhojpur-table--bulking tbody tr',
        CLASS_IS_UNDO = 'is_undo',
        CLASS_TABLE_MDL = 'mdl-data-table--selectable',
        CLASS_SLIDEOUT = '.bhojpur-slideout',
        ACTION_FORM_DATA = 'primary_values[]',
        CLASS_HEADER_TOGGLE = '.bhojpur-page__header .bhojpur-actions, .bhojpur-page__header .bhojpur-search-container',
        CLASS_BODY_LOADING = ".bhojpur-body__loading";

    function BhojpurAction(element, options) {
        this.$element = $(element);
        this.options = $.extend({}, BhojpurAction.DEFAULTS, $.isPlainObject(options) && options);
        this.ajaxForm = {};
        this.init();
    }

    BhojpurAction.prototype = {
        constructor: BhojpurAction,
        ...
        bind: function() {
            this.$element.on(EVENT_CLICK, '.bhojpur-action--bulk', this.renderBulkTable.bind(this)).on(EVENT_CLICK, '.bhojpur-action--exit-bulk', this.removeBulkTable.bind(this));
        ...
        initActions: function() {
            if (!$(CLASS_TABLE).find('table').length) {
                ...
                $('.bhojpur-page__header a.bhojpur-action--button').hide();
            }
        },
        ...
        renderBulkTable: function() {
            ...
            if ($body.hasClass('bhojpur-slideout-open')) {
                $body.data('bhojpur.slideout').hide();
            }

            $('.bhojpur-table__inner-list').remove();
        ...
        toggleBulkButtons: function() {
            ...
            $(CLASS_TABLE)
                .toggleClass('bhojpur-table--bulking')
                .find('.bhojpur-table__actions')
                .toggle();
        ...
        renderFlashMessage: function(data) {
            let flashMessageTmpl = BhojpurAction.FLASHMESSAGETMPL;
        ...
        addLoading: function() {
          ...
          var $loading = $(BhojpurAction.TEMPLATE_LOADING);
        ...
        submit: function($actionButton) {
            ...
            if($actionButton.hasClass("bhojpur-action-disabled")){
                return false;
            }

            if (properties.fromIndex && (!ajaxForm.formData || !ajaxForm.formData.length)) {
                BHOJPUR.bhojpurConfirm(ajaxForm.properties.errorNoItem);
                return;
            }

            if (properties.confirm) {
                BHOJPUR.bhojpurConfirm(properties, function(confirm) {
                    if (confirm) {
        ...
        handleAjaxSubmit: function(ajaxForm, $actionButton) {
            let _this = this,
                ...
                $parent = $actionButton.closest(".bhojpur-action-forms"),
                properties = ajaxForm.properties || $actionButton.data(),
                url = properties.url,
            ...
            this.addLoading();
            if($parent.length){
                $parent.find('[data-ajax-form="true"][data-method]').addClass("bhojpur-action-disabled");
            } else {
                $actionButton.addClass("bhojpur-action-disabled");
            }
            ...
            $.ajax(url, {
                ...
                complete: function(response) {
                    ...
                    $actionButton.prop('disabled', false);
                    if($parent.length){
                        $parent.find('[data-ajax-form="true"][data-method]').removeClass("bhojpur-action-disabled");
                    } else {
                        $actionButton.removeClass("bhojpur-action-disabled");
                    }
                    ...
                    if (disposition && disposition.indexOf('attachment') !== -1) {
                        ...
                        BHOJPUR.bhojpurAjaxHandleFile(url, contentType, fileName, fileData);
        ...
        switchButtons: function($element, disbale) {
            ...
            $element.find('.bhojpur-action-button').prop('disabled', needDisbale);
        },
    ...
    BhojpurAction.DEFAULTS = {};

    BhojpurAction.TEMPLATE_LOADING = `<div class="bhojpur-body__loading">
    ...
    $.fn.bhojpurSliderAfterShow.bhojpurInsertActionData = function(url, html) {
        let $action = $(html).find('[data-toggle="bhojpur-action-slideout"]'),
    ...
    BhojpurAction.plugin = function(options) {
        return this.each(function() {
            ...
            if (!data) {
                $this.data(NAMESPACE, (data = new BhojpurAction(this, options)));
            }
    ...
    $(function() {
        let options = {},
            selector = '[data-toggle="bhojpur.action.bulk"]';

        if (!$(selector).length) {
            $(document).on(EVENT_CLICK, CLASS_MENU_ACTIONS, function(e) {
                new BhojpurAction().actionSubmit($(e.target));
                return false;
            });
        }
        ...
        $(document)
            .on(EVENT_DISABLE, function(e) {
                BhojpurAction.plugin.call($(selector, e.target), 'destroy');
            })
            .on(EVENT_ENABLE, function(e) {
                BhojpurAction.plugin.call($(selector, e.target), options);
            })
            .triggerHandler(EVENT_ENABLE);
    });

    return BhojpurAction;
});