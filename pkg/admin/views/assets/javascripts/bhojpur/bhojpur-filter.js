(function(factory) {
    ...
    function BhojpurFilter(element, options) {
        this.$element = $(element);
        this.options = $.extend({}, BhojpurFilter.DEFAULTS, $.isPlainObject(options) && options);
        this.init();
    }

    BhojpurFilter.prototype = {
        constructor: BhojpurFilter,
    ...
    BhojpurFilter.DEFAULTS = {
        label: false,
        group: false
    };

    BhojpurFilter.plugin = function(options) {
        return this.each(function() {
            ...
            if (!data) {
                ...
                $this.data(NAMESPACE, (data = new BhojpurFilter(this, options)));
            }
    ...
    $(function() {
        var selector = '[data-toggle="bhojpur.filter"]';
        var options = {
            label: 'a',
            group: 'select'
        };
        ...
        $(document)
            .on(EVENT_DISABLE, function(e) {
                BhojpurFilter.plugin.call($(selector, e.target), 'destroy');
            })
            .on(EVENT_ENABLE, function(e) {
                BhojpurFilter.plugin.call($(selector, e.target), options);
            })
            .triggerHandler(EVENT_ENABLE);
    });

    return BhojpurFilter;
});