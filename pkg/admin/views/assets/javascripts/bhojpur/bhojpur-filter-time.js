(function(factory) {
    ...
    let location = window.location,
        $document = $(document),
        NAMESPACE = 'bhojpur.filter',
        EVENT_FILTER_CHANGE = 'filterChanged.' + NAMESPACE,
        EVENT_ENABLE = 'enable.' + NAMESPACE,
        EVENT_DISABLE = 'disable.' + NAMESPACE,
        EVENT_CLICK = 'click.' + NAMESPACE,
        CLASS_BOTTOMSHEETS = '.bhojpur-bottomsheets',
        CLASS_DATE_START = '.bhojpur-filter__start',
        CLASS_DATE_END = '.bhojpur-filter__end',
        CLASS_SEARCH_PARAM = '[data-search-param]',
        CLASS_FILTER_SELECTOR = '.bhojpur-filter__dropdown',
        CLASS_FILTER_TOGGLE = '.bhojpur-filter-toggle',
        CLASS_IS_SELECTED = 'is-selected';

    function BhojpurFilterTime(element, options) {
        this.$element = $(element);
        this.options = $.extend({}, BhojpurFilterTime.DEFAULTS, $.isPlainObject(options) && options);
        this.init();
    }

    BhojpurFilterTime.prototype = {
        constructor: BhojpurFilterTime,
        ...
        initActionTemplate: function() {
            ...
            if (scheduleStartAt || scheduleEndAt) {
                ...
                $filterToggle
                    ...
                    .find('.bhojpur-selector-label')
                    .html(scheduleStartAt + scheduleEndAt);
                $filterToggle.append('<i class="material-icons bhojpur-selector-clear">clear</i>');
            }
        },
        ...
        close: function(e) {
            var $target = $(e.target),
                ...
                isInModal = $target.closest('.bhojpur-modal').length,
        ...
        clear: function() {
            var $trigger = $(this.options.trigger),
                $label = $trigger.find('.bhojpur-selector-label');
    ...
    BhojpurFilterTime.plugin = function(options) {
        return this.each(function() {
            ...
            if (!data) {
                ...
                $this.data(NAMESPACE, (data = new BhojpurFilterTime(this, options)));
            }
    ...
    $(function() {
        var selector = '[data-toggle="bhojpur.filter.time"]';
        var options = {
            label: '.bhojpur-filter__block-buttons button',
            trigger: 'a.bhojpur-filter-toggle',
            button: '.bhojpur-filter__button-search',
            clear: '.bhojpur-selector-clear'
        };

        $(document)
            .on(EVENT_DISABLE, function(e) {
                BhojpurFilterTime.plugin.call($(selector, e.target), 'destroy');
            })
            .on(EVENT_ENABLE, function(e) {
                BhojpurFilterTime.plugin.call($(selector, e.target), options);
            })
            .triggerHandler(EVENT_ENABLE);
    });

    return BhojpurFilterTime;
});