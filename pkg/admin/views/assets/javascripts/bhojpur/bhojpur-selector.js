(function(factory) {
    ...
    var NAMESPACE = 'bhojpur.selector';
    ...
    var SELECTOR_TOGGLE = '.bhojpur-selector-toggle';
    var SELECTOR_LABEL = '.bhojpur-selector-label';
    var SELECTOR_CLEAR = '.bhojpur-selector-clear';
    var SELECTOR_MENU = '.bhojpur-selector-menu';
    var CLASS_BOTTOMSHEETS = '.bhojpur-bottomsheets';

    function BhojpurSelector(element, options) {
        this.options = options;
        this.$element = $(element);
        this.init();
    }

    BhojpurSelector.prototype = {
        constructor: BhojpurSelector,
        ...
        build: function() {
            var $this = this.$element;
            var $selector = $(BhojpurSelector.TEMPLATE);
        ...
        pick: function(data, initialized) {
            ...
            if (!initialized) {
                ...
                if ($element.closest(CLASS_BOTTOMSHEETS).length && !$element.closest('[data-toggle="bhojpur.filter"]').length) {
                    // If action is in bottom sheet, will trigger filterChanged.bhojpur.selector event, add passed data.value parameter to event.
    ...
    BhojpurSelector.DEFAULTS = {
        aligned: 'left',
        clearable: false
    };

    BhojpurSelector.TEMPLATE =
        '<div class="bhojpur-selector">' +
        '<a class="bhojpur-selector-toggle">' +
        '<span class="bhojpur-selector-label"></span>' +
        '<i class="material-icons bhojpur-selector-arrow">arrow_drop_down</i>' +
        '<i class="material-icons bhojpur-selector-clear">clear</i>' +
        '</a>' +
        '<ul class="bhojpur-selector-menu"></ul>' +
        '</div>';

    BhojpurSelector.plugin = function(option) {
        return this.each(function() {
            ...
            if (!data) {
                ...
                options = $.extend({}, BhojpurSelector.DEFAULTS, $this.data(), typeof option === 'object' && option);
                $this.data(NAMESPACE, (data = new BhojpurSelector(this, options)));
            }
    ...
    $(function() {
        var selector = '[data-toggle="bhojpur.selector"]';

        $(document)
            .on(EVENT_DISABLE, function(e) {
                BhojpurSelector.plugin.call($(selector, e.target), 'destroy');
            })
            .on(EVENT_ENABLE, function(e) {
                BhojpurSelector.plugin.call($(selector, e.target));
            })
            .triggerHandler(EVENT_ENABLE);
    });

    return BhojpurSelector;
});