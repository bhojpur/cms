# Bhojpur CMS - Render Engine

It provides handy controls when rendering templates.

## Usage

### Initialize [Render](https://github.com/bhojpur/cms/pkg/render)
...
The `Execute` function accepts four parameters:

1. The template name. In this example, [Render](https://github.com/bhojpur/cms/pkg/render) will look up template `index.tmpl` from view paths. the default view path is `{current_repo_path}/app/views`, and you could register more view paths.
...
3. [http.Request](https://golang.org/pkg/net/http/#Request).
4. [http.ResponseWriter](https://golang.org/pkg/net/http/#ResponseWriter).

### Understanding `yield`

`yield` is a func that could be used in layout views, it will render current specified template. For above example, think of `yield` as a placeholder, and it will replaced with template `index.tmpl`'s content.
...
[Render](https://github.com/bhojpur/cms/pkg/render) will find the layout at `{current_repo_path}/app/views/layouts/new_layout.tmpl`.

### Render with helper functions

Sometimes you may want to have some helper functions in your template. [Render](https://github.com/bhojpur/cms/pkg/render) supports passing helper functions by `Funcs` function.
...
Put the [Render](https://github.com/bhojpur/cms/pkg/render) inside [Responder](./responder.md) handle function like this.
...
### Use with [cmsctl](../plugins/assetfs.md)

```go
$ cmsctl generate --exit-after-compile=false config/views
