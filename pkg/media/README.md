# Bhojpur CMS - Media Library

The Media is a [Go](http://golang.org/) library that supports the upload of *files*/*images*/*videos* to a filesystem or cloud storage as well as *linked videos* (i.e. YouTube, Vimeo, etc.). The plugin includes:
...
Media depends on [Bhojpur ORM](https://github.com/bhojpur/orm) models as it is using [Bhojpur ORM](https://github.com/bhojpur/orm)'s callbacks to handle file processing, so you will need to register callbacks first:

```go
import (
  "github.com/bhojpur/orm/pkg/engine"
  "github.com/bhojpur/cms/pkg/media"
)

DB, err = orm.Open("sqlite3", "demo_db") // [Bhojpur ORM](https://github.com/bhojpur/orm)

media.RegisterCallbacks(DB)
```

Then, add [Object Storage Service](https://github.com/bhojpur/cms/pkg/media/oss) to your model:
...
Last, configure the storage. The default value is `oss.Storage := filesystem.New("public")`. Here we configure Amazon S3 as a storage.
...
oss.Storage := s3.New(s3.Config{AccessID: "access_id", AccessKey: "access_key", Region: "region", Bucket: "bucket", Endpoint: "static.bhojpur.net", ACL: aws.BucketCannedACLPublicRead})
// Default configuration `oss.Storage := filesystem.New("public")`
```

## Operate stored files

The [Object Storage Service](https://github.com/bhojpur/cms/pkg/media/oss) provides a pretty simple API to operate files on filesytem or cloud storage
...
```go
storage := s3.New(s3.Config{AccessID: "access_id", AccessKey: "access_key", Region: "region", Bucket: "bucket", Endpoint: "static.bhojpur.net", ACL: aws.BucketCannedACLPublicRead})
...
"github.com/bhojpur/cms/pkg/media/oss"
"github.com/oss/s3"
"github.com/bhojpur/cms/pkg/media/media_library"
)
...
The files under `public` will be exposed to public(especially the search engine!), Imagine someone upload an illegal or sensitive file to your server. The directory is fully visible to everyone and its indexable by search engines and boom!

To avoid this problem, we made a safer `FileServer` function [here](https://github.com/bhojpur/cms/blob/master/utils/utils.go). This function serves file only. So the previous code now turned into:
...
The Media Library has some features aimed at helping achieve Accessibile frontends:
