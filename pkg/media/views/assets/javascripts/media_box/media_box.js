(function(factory) {
    ...
    let $body = $('body'),
        $document = $(document),
        NAMESPACE = 'bhojpur.medialibrary.select',
        PARENT_NAMESPACE = 'bhojpur.bottomsheets',
        EVENT_CLICK = 'click.' + NAMESPACE,
        EVENT_ENABLE = 'enable.' + NAMESPACE,
        EVENT_DISABLE = 'disable.' + NAMESPACE,
        EVENT_RELOAD = 'reload.' + PARENT_NAMESPACE,
        CLASS_SELECT_ICON = '.bhojpur-select__select-icon',
        CLASS_SELECT_HINT = '.bhojpur-selectmany__hint',
        CLASS_PARENT = '.bhojpur-field__mediabox',
        CLASS_LISTS = '.bhojpur-field__mediabox-list',
        CLASS_ITEM = '.bhojpur-field__mediabox-item',
        CLASS_LISTS_DATA = '.bhojpur-field__mediabox-data',
        CLASS_SELECTED = 'is_selected',
        CLASS_DELETE = 'is_deleted',
        CLASS_CROPPER_OPTIONS = 'textarea.bhojpur-file__options',
        CLASS_CROPPER_DELETE = '.bhojpur-cropper__toggle--delete',
        CLASS_CROPPER_UNDO = '.bhojpur-cropper__toggle--undo',
        CLASS_MEDIABOX = 'bhojpur-bottomsheets__mediabox';
    ...
    function BhojpurMediaBox(element, options) {
        this.$element = $(element);
        this.options = $.extend({}, BhojpurMediaBox.DEFAULTS, $.isPlainObject(options) && options);
        this.init();
    }

    BhojpurMediaBox.prototype = {
        constructor: BhojpurMediaBox,
        ...
        bind: function() {
            ...
            this.$element
                ...
                .on('change.bhojpur.cropper', CLASS_CROPPER_OPTIONS, this.imageCrop.bind(this));
        },

        unbind: function() {
            $document.off(EVENT_RELOAD, `.${CLASS_MEDIABOX}`);

            this.$element.off(EVENT_CLICK).off('change.bhojpur.cropper');
        },
        ...
        undoDeleteSelected: function(e) {
            ...
            $selectFeild
                ...
                .show();
            this.updateMediaLibraryData($target.closest(CLASS_LISTS));
            $target.closest('.bhojpur-fieldset__alert').remove();
        ...
        initMedia: function() {
            ...
            $trs.each(function() {
                $tr = $(this);
                $img = $tr.find('.bhojpur-table--ml-slideout p img').first();
                $tr.find('.bhojpur-table__actions').remove();
                if ($img.length) {
                    $tr.find('.bhojpur-table--medialibrary-item').css('background-image', 'url(' + $img.prop('src') + ')');
                    $img.parent().remove();
                }
            });
        ...
        updateHint: function(data) {
            ...
            this.$bottomsheets.find('.bhojpur-page__body').before(template);
        },
        ...
        changeIcon: function($ele, isNew) {
            var $item = $ele.find('.bhojpur-table--medialibrary-item'),
        ...
        syncImageCrop: function($ele, callback) {
            ...
            $.ajax({
                ...
                complete: function() {
                    ...
                    $ele.find('.bhojpur-media-loading').remove();
                }
            });
        },
        ...
        showHiddenItem: function($hiddenItem) {
            $hiddenItem
                ...
                .find('.bhojpur-file__list')
                .show();
            $hiddenItem.find('.bhojpur-fieldset__alert').remove();
        },
        ...
        addItem: function(data, isNewData) {
            let $template = $(window.Mustache.render(this.TEMPLATE_IMAGE, data)),
                $input = $template.find('.bhojpur-file__input'),
            ...
            if (hasNewCropSize && $input.data('bhojpur.cropper') && !isSVG) {
                $input.data('bhojpur.cropper').load(data.MediaOption.URL, true, function() {
                    _this.syncImageCrop($item, _this.resetImages);
                });
            }

            this.$bottomsheets.find('.bhojpur-media-loading').remove();
        ...
        handleSelectMany: function($bottomsheets) {
            ...
            $bottomsheets.bhojpurSelectCore(options).addClass(CLASS_MEDIABOX);
        ...
        addMediaLoading: function($ele) {
            $('<div class="bhojpur-media-loading"><div class="mdl-spinner mdl-spinner--single-color mdl-js-spinner is-active"></div></div>')
                .appendTo($ele)
    ...
    BhojpurMediaBox.plugin = function(options) {
        return this.each(function() {
            ...
            if (!data) {
                ...
                $this.data(NAMESPACE, (data = new BhojpurMediaBox(this, options)));
            }
    ...
    $(function() {
        var selector = '[data-toggle="bhojpur.mediabox"]';
        $(document)
            .on(EVENT_DISABLE, function(e) {
                BhojpurMediaBox.plugin.call($(selector, e.target), 'destroy');
            })
            .on(EVENT_ENABLE, function(e) {
                BhojpurMediaBox.plugin.call($(selector, e.target));
            })
            .triggerHandler(EVENT_ENABLE);
    });

    return BhojpurMediaBox;
});